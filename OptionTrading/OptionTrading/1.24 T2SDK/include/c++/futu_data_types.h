...
//相信请参考示例demo
#define HSFUSDK_VERSION     0x10000024
#define HSFUSDK_VERSTRING   "V1.0.0.24" 
...
{
	NAType          = -1,     // 未知类型
	SingleCode      = 6,      // 单腿行情 //行情订阅类型由原来的0调整为6 
	RspReport       = 1,      // 委托回报
	CombinCode      = 2,      // 组合行情
	OnlineMsg       = 3,      // 在线消息
	ExchStatus      = 8       // 交易所状态
...
enum FUTU_MSG_TYPE
{
	...
	//20121025 tangrj 接口增加当日资金流水查询和持仓明细查询 修改单：20120731009,20120731008
	MSG_TYPE_GET_BANK_JOUR         = 226, //查询当日资金流水
	MSG_TYPE_GET_HOLDSINFO_DETAIL  = 227, //查询持仓明细

	//20131212 zhoukm 新增期权功能 修改单:20131217031
	MSG_TYPE_GET_OPTION_HOLDS_EXPIRE = 228,//查询到期执行持仓信息
	MSG_TYPE_EXT_OPTION_GIVEUPEXEC_APPLY = 229,//放弃执行申请

	//20140120 zhoukm 修改单：20131217031
	MSG_TYPE_EXT_OPTION_DROP_EXECUTE_FLOWS = 230//查期权单笔持仓执行流水
};
...
struct CMarketInfo
{
	char            contract_code[30];  //0 合约代码 //单腿行情合约代码由原来的13调整为30，其他不变
	...
	unsigned int    update_time;        //46 更新时间
};
...
struct COrderRspInfo
{
	...
	char       entrust_occasion[33];  //32 委托场景
	//20130828 chends 回报支持futu_entrust_price2字段 修改单：20130821047
	double futu_entrust_price2;		  //33 委托价格2
	//20120428 tanghui
	char		 error_message[255];    //34 错误信息
};
...
struct CRealRspInfo
{
	...
	char       entrust_occasion[33];  //29 委托场景
	//20130828 chends 回报支持futu_entrust_price2字段 修改单：20130821047
	double futu_entrust_price2;		 //30 委托价格2
	//20120428 tanghui 委托反馈和成交反馈增加错误信息字段 修改单:20120412049
	char		 error_message[255];    //31 错误信息
};

//交易所状态反馈信息 
struct CExchStatusInfo
{
	char futu_exch_type[3];  //交易所,F1,F2,F3,F4
	char futu_status[3];     //状态,0连接断开,1开盘前, 2集合报价 3竞价平衡 4竞价撮合 5 连续交易 6暂停交易 7闭市 X未知
	int  can_trade;          //是否可以交易,1可交易,0不可交易
};
#pragma pack(pop)
